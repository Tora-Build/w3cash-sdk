...
import { WaitAdapter } from "../src/poca/adapters/WaitAdapter.sol";
import { AaveAdapter } from "../src/poca/adapters/AaveAdapter.sol";

/**
 * @title DeployPoca
 * @notice Deployment script for POCA architecture (Option 4: Immutable Processor + Upgradeable Registry)
 *
 * Deployment Order:
 * 1. Deploy AdapterRegistry (upgradeable, owned)
 * 2. Deploy PocaProcessor with registry reference (immutable)
...
contract DeployPoca is Script {
    ...
    uint8 constant AAVE_ADAPTER_ID = 1;
    ...
    function run() external {
        ...
        // 2. Deploy PocaProcessor (immutable, references registry)
        PocaProcessor processor = new PocaProcessor(address(registry));
        console.log("PocaProcessor deployed at:", address(processor));
        ...
        console.log("PocaProcessor:", address(processor));
        console.log("");
