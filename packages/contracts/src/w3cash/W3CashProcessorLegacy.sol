...
/**
 * @title W3CashProcessorLegacy
 * @notice W3Cash v4 - On-chain automation layer for AI agents
 * @dev Executes signed payloads with PAUSE_EXECUTION support for conditional/scheduled actions
 *
 * Key features:
 * - SignedPayload verification (agent signs once, execution can happen later)
 * - Nonce-based cancellation (incrementNonce() invalidates all pending flows)
 * - PAUSE_EXECUTION pattern (non-reverting condition checks)
 * - Action/Condition plugin architecture
 ...
 */
contract W3CashProcessorLegacy {
    ...
    /// @notice User nonces for cancellation (increment to invalidate all pending flows)
    ...
    function execute(bytes memory encodedSignedPayload) external payable {
        ...
        _verifySignature(sp);
        _execute(sp.instruction, sp.initiator, sp.nonce, sp.signature);
    }
    ...
    /**
     * @notice Cancel all pending flows by incrementing nonce
     * @dev Any signed payload with the old nonce becomes invalid
     * @return newNonce The user's new nonce
     */
    function incrementNonce() external returns (uint256 newNonce) {
        uint256 oldNonce = nonces[msg.sender];
        newNonce = oldNonce + 1;
        nonces[msg.sender] = newNonce;
        emit NonceCancelled(msg.sender, oldNonce, newNonce);
    }

    // --- Internal Functions ---

    function _verifySignature(DataTypes.SignedPayload memory sp) internal view {
        // Check nonce matches current (allows cancellation by incrementing)
        if (sp.nonce != nonces[sp.initiator]) revert Errors.InvalidNonce();

        (, bytes memory payload) = _splitHeaderAndPayload(sp.instruction);
        // Sign over (payloadHash, nonce) to bind signature to specific nonce
        bytes32 messageHash = keccak256(abi.encodePacked(keccak256(payload), sp.nonce));
        ...
        for (; seq < length;) {
            ...
            if (registry.getChain(chain) != block.chainid) {
                ...
                _sendCrossChainMessage(
                    instruction, initiator, nonce, signature, 
